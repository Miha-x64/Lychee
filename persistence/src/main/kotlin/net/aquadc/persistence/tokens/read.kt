...
internal/*accessed from a class*/ fun TokenStream.nextField(sch: Schema<*>): Int {
...
internal class TokensIterator<SCH : Schema<SCH>, T>(
    ...
    private fun readTransient(): Struct<SCH> {
        ...
        while (nextField != -1) { // this is ultra unhandy without assignment as expression
            fields = fields.forceAddField(nextField, schema)
            values[nextField] = tokens.readAs(schema.typeAt(nextField))
