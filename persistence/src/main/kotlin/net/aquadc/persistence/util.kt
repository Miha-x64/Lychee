...
internal fun <SCH : Schema<SCH>> fill(builder: StructBuilder<SCH>, schema: SCH, fields: FieldSet<SCH, *>, values: Any?) {
    fields as FieldSet<SCH, FieldDef<SCH, *, *>>
    when (fields.size) {
        0 -> {
...
fun <SCH : Schema<SCH>> PartialStruct<SCH>.fieldValues(): Any? {
    ...
    return when (val fieldCount = fields.size) {
        0 -> null
        1 -> {
//          55: invokestatic  net/aquadc/persistence/struct/FieldSetKt."single-30ZEADk":(Lnet/aquadc/persistence/struct/Schema;J)I
//          58: invokestatic  java/lang/Integer.valueOf:(I)Ljava/lang/Integer;
//          61: checkcast     class net/aquadc/persistence/struct/FieldDef
//          64: invokevirtual Method net/aquadc/persistence/struct/FieldDef."unbox-impl":()I
            TODO("ClassCastException")
            getOrThrow(schema.single(fields))
        }
        else -> {
...
inline fun <T, SCH : Schema<SCH>> readPartial(
    ...
    maybeReadNextField: () -> /*ordinal | -1*/ Int,
    ...
    if (firstField >= 0) {
        fields = fields or (1L shl firstField)
...
@RestrictTo(RestrictTo.Scope.LIBRARY_GROUP)
fun <SCH : Schema<SCH>> Long.forceAddField(ordinal: Int, schema: SCH): Long {
    val newFields = this or (1L shl ordinal)
    if (this == newFields) {
...
internal fun <SCH : Schema<SCH>> gatherValues(fields: FieldSet<SCH, FieldDef<SCH, *, *>>, fieldValues: ArrayList<Any?>): Array<Any?> {
    ...
    repeat(fieldCount) { _ ->
        ...
        val field = FieldDef<SCH, Any?, DataType<Any?>>(fieldValues.pop() as Int)
